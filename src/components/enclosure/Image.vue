<template>
  <img
    :src="image"
    :title="title"
    :alt="alt"
    @error="failover"
    @load="loading = false"
  >
</template>

<script setup>
import { ref, toRefs, onMounted, watch } from 'vue'
import exifImageUrl from '@/utils/DataImage'
import { useProp } from '@/composables/useProp.js'
import { decode } from 'blurhash'
import loadImage from 'blueimp-load-image'
import { computedAsync } from '@vueuse/core'

const hashImage = computedAsync(
  async () => {
    if (blurhash.value && width.value && height.value) {
      const pixels = decode(blurhash.value, width.value, height.value)

      const imageData = await loadImage(
        pixels,
        canvas => {
          if (canvas.type === 'error') {
            return Promise.reject(Error('can not load blur image'))
          }
          return canvas.toDataURL('image/webp')
        },
        {
          canvas: true,
          meta: true,
          orientation: true
        }
      )
      return Promise.resolve(imageData)
    }
  },
  null
)
const props = defineProps({
  src: useProp(String),
  title: useProp(String),
  alt: useProp(String),
  blurhash: useProp(String),
  width: useProp(Number),
  height: useProp(Number),
  readExif: useProp(Boolean),
  failoverSrc: useProp(String, 'data:image/png;base64,')
})

const { src, readExif, failoverSrc, computed, blurhash, width, height } = toRefs(props)
const loading = ref(false)
const originalSrc = ref(src.value)

// 1500 x 500

onMounted(async () => {
  if (readExif.value) {
    try {
      const transformed = await exifImageUrl(src.value)
      originalSrc.value = transformed
    } catch (err) {
      console.warn(err)
    }
  }
})

const image = computed(() => loading.value && hashImage.value ? hashImage.value : originalSrc.value)

watch(src, async (newSrc, _oldSrc) => {
  originalSrc.value = newSrc
  if (readExif.value) {
    try {
      const transformed = await exifImageUrl(newSrc)
      originalSrc.value = transformed
    } catch (err) {}
  }
})

const failover = () => {
  originalSrc.value = failoverSrc.value
}

</script>
